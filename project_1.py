...
train_filename=rgbset2+rgbset3+rgbset4+rgbset1+rgbset7+rgbset8+rgbset6
train_depth=depset2+depset3+depset4+depset1+depset7+depset8+depset6
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.high_conv4=Unit(32,64,(3,5,3),(2,2,1))
    ...
    def forward(self,Hinput):
        ...
        low_output=self.ld1(low_output)
        low_output=F.log_softmax(low_output,dim=1)
        
        high_output=self.high_net(Hinput)
        high_output=high_output.view(-1,64*2*3*2)
        high_output=self.high_fc1(high_output)
        high_output=self.high_fc2(high_output)
        high_output=self.high_fc3(high_output)
        high_output=self.hd1(high_output)
        high_output=F.log_softmax(high_output,dim=1)
...
if __name__ == "__main__":
    ...
    train_acc_plt,train_loss_plt,valid_acc_plt,valid_loss_plt,total_time=train(2)
    f=open("project_13_5_dr.txt",'w')
